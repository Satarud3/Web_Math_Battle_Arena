  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const qType = (qData.type || "MULTIPLE_CHOICE") as QuestionType;
        setType(qType);
        ...
        // Map values according to type
        if (qType === "MULTIPLE_CHOICE") {
          setOptionA(qData.options?.A || "");
          setOptionB(qData.options?.B || "");
          setOptionC(qData.options?.C || "");
          setOptionD(qData.options?.D || "");
          setCorrectAnswer(qData.correctAnswer || "A");
        } else if (qType === "SELECT_MULTIPLE") {
          setOptionA(qData.options?.A || "");
          setOptionB(qData.options?.B || "");
          setOptionC(qData.options?.C || "");
          setOptionD(qData.options?.D || "");
          const corrects = (qData.correctAnswer || "").split(",");
          const checks = {
            A: corrects.includes("A"),
            B: corrects.includes("B"),
            C: corrects.includes("C"),
            D: corrects.includes("D")
          };
          setSelectedMultipleAnswers(checks);
          setCorrectAnswer(qData.correctAnswer || "");
        } else if (qType === "TRUE_FALSE") {
          setCorrectAnswer(qData.correctAnswer || "TRUE");
        } else if (qType === "FILL_BLANK") {
          setCorrectAnswer(qData.correctAnswer || "");
        } else {
          setCorrectAnswer(qData.correctAnswer || "");
          setOptionsJson(qData.options ? JSON.stringify(qData.options, null, 2) : "");
          setQuestionDataJson(qData.questionData ? JSON.stringify(qData.questionData, null, 2) : "");
          setAnswerDataJson(qData.answerData ? JSON.stringify(qData.answerData, null, 2) : "");
        }
      } catch (error: unknown) {
    ...
    <div className="p-6 sm:p-8 flex flex-col gap-6 max-w-5xl mx-auto w-full">
      ...
      <div>
        ...
        <p className="text-xs text-slate-400 mt-1">
          Ubah tipe soal matematika, modifikasi pertanyaan, parameter JSON interaktif, atau matikan keaktifan soal ini.
        </p>
      </div>

      {/* Form Element */}
      {loadingData ? (
        <div className="bg-[#0E131F]/80 border border-white/10 rounded-2xl p-8 space-y-6 shadow-2xl animate-pulse">
          <div className="h-8 bg-[#131A26] rounded-xl w-1/3" />
          <div className="h-24 bg-[#131A26] rounded-xl" />
          <div className="grid grid-cols-2 gap-4">
            <div className="h-10 bg-[#131A26] rounded-xl" />
            <div className="h-10 bg-[#131A26] rounded-xl" />
          </div>
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="bg-[#0E131F]/80 backdrop-blur border border-white/10 rounded-2xl p-6 flex flex-col gap-6 shadow-2xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-2 border-t border-white/5 pt-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">
