...
import { 
  ...
  ArrowLeft, ShieldAlert, Loader2, Play, Swords
} from "lucide-react";
  ...
  useEffect(() => {
    ...
    const fetchPublicProfileData = async () => {
      try {
        ...
        // 2. Fetch match history using user ID retrieved from profile
        if (profileData?.id) {
  ...
  return (
    <div className="relative min-h-screen bg-bg-main text-white flex flex-col font-sans">
      ...
      <main className="flex-grow max-w-5xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {error || !profile ? (
          <div className="p-6 bg-red-500/10 border border-red-500/20 rounded-3xl text-red-200 flex flex-col items-center gap-3 text-sm text-center max-w-md mx-auto">
            <ShieldAlert className="w-8 h-8 text-red-400 shrink-0" />
            <h3 className="font-bold text-base">Terjadi Kesalahan</h3>
            <p className="text-slate-400 text-xs">{error || "Profil ksatria tidak ditemukan."}</p>
            <Link href="/leaderboard" className="mt-4 px-4 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 text-xs font-bold rounded-xl transition-all">
              Kembali
            </Link>
          </div>
        ) : (
          <>
            {/* Profile Card Header */}
            <div className="bg-[#0E1524]/60 backdrop-blur border border-slate-800 rounded-3xl p-6 sm:p-8 mb-8 flex flex-col md:flex-row items-center gap-6 relative overflow-hidden shadow-2xl">
              ...
              <div className="text-center md:text-left min-w-0 flex-grow">
                ...
                <p className="text-slate-400 mt-1 font-medium text-sm">@{profile.username}</p>
                <div className="mt-3 flex flex-wrap justify-center md:justify-start gap-2">
                  ...
                  {profile.ranking && (
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 uppercase tracking-wider">
                      Rank #{profile.ranking.currentRank || "Unranked"}
                    </span>
                  )}
                </div>
              </div>
              ...
              {profile.ranking && (
                <div className={`md:ml-auto w-full md:w-auto p-5 rounded-2xl border bg-gradient-to-br flex flex-col items-center justify-center text-center ${getTierGlowClass(profile.ranking.tier)}`}>
                  <Award className={`w-10 h-10 ${
                    profile.ranking.tier.toLowerCase() === "master" ? "text-purple-400 animate-pulse" :
                    profile.ranking.tier.toLowerCase() === "platinum" ? "text-cyan-400" :
                    profile.ranking.tier.toLowerCase() === "gold" ? "text-yellow-400" : "text-slate-300"
                  }`} />
                  <div className="text-[10px] font-extrabold uppercase text-slate-400 tracking-widest mt-1">Tier Saat Ini</div>
                  <div className="text-xl font-black text-slate-200 mt-0.5">{profile.ranking.tier}</div>
                  <div className="text-sm font-black text-indigo-400 mt-1.5 flex items-center gap-1">
                    {profile.ranking.ratingPoint} <span className="text-[9px] font-bold text-slate-500">RP</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
              ...
              <div className="bg-[#0E1524]/60 backdrop-blur border border-slate-800 rounded-2xl p-5 shadow-lg">
                <div className="text-slate-500 text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 mb-2">
                  <Swords className="w-4 h-4 text-indigo-400" />
                  Total Match
                </div>
                <div className="text-2xl sm:text-3xl font-black text-slate-100">{profile.userStats?.totalMatches || 0}</div>
              ...
              <div className="bg-[#0E1524]/60 backdrop-blur border border-slate-800 rounded-2xl p-5 shadow-lg">
                ...
                <div className="text-2xl sm:text-3xl font-black text-slate-100">
                  {profile.userStats ? parseFloat(profile.userStats.winRate).toFixed(1) : "0.0"}%
                </div>
              ...
              <div className="bg-[#0E1524]/60 backdrop-blur border border-slate-800 rounded-2xl p-5 shadow-lg">
                ...
                <div className="text-2xl sm:text-3xl font-black text-slate-100">
                  {profile.userStats ? parseFloat(profile.userStats.accuracy).toFixed(1) : "0.0"}%
                </div>
                <div className="text-[10px] text-slate-500 mt-1">Rasio Jawaban Benar</div>
              </div>

              <div className="bg-[#0E1524]/60 backdrop-blur border border-slate-800 rounded-2xl p-5 shadow-lg flex flex-col justify-between">
                <div className="text-slate-500 text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 mb-2">
                ...
                <div className="grid grid-cols-3 text-center text-xs gap-1 mt-1">
                  <div className="bg-emerald-500/10 border border-emerald-500/20 py-1 rounded text-emerald-400">
                    <span className="block font-black text-sm">{profile.userStats?.totalWins || 0}</span>
                    W
                  </div>
                  <div className="bg-red-500/10 border border-red-500/20 py-1 rounded text-red-400">
                    <span className="block font-black text-sm">{profile.userStats?.totalLosses || 0}</span>
                    L
                  </div>
                  <div className="bg-slate-500/10 border border-slate-500/20 py-1 rounded text-slate-400">
                    <span className="block font-black text-sm">{profile.userStats?.totalDraws || 0}</span>
                    D
                  </div>
                </div>
              </div>
            ...
            <div className="bg-[#0E1524]/60 backdrop-blur border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl">
